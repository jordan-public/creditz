...
| Mode | Prover execution | Verifier execution | Ledger / nullifier enforcement | Notes |
| --- | --- | --- | --- | --- |
| Database ledger | Backend API, via `/api/provekit/prove`. The World Mini App sends the local note witness to the backend for the hackathon demo. | Backend API, via `/api/spend` and the configured ProveKit verifier command. | Backend database. Commitments and spent nullifiers are stored in SQLite, with uniqueness preventing replay. | Fastest local mode. The Mini App orchestrates the flow, but the private prover and verifier do not execute inside World App. |
| Off-chain verifier with on-chain storage | Backend API, via `/api/provekit/prove`. | Backend API. After verification, the backend signs an attestation. | World Chain contract. `CreditRegistry.sol` stores commitments, spent nullifiers, issuer records, and merchant settlement records. | This is the working on-chain MVP pattern: proof verification is off-chain, while nullifier and commitment state are recorded on-chain. |
| Failed on-chain-verifier experiment | Backend API generated EVM-style proof bytes for the spend statement. | Intended to be on-chain in a generated Solidity UltraHonk verifier, called from `CreditRegistry.sol`. | Intended to be World Chain contract state. | This did not deploy because the generated verifier bytecode was about 27.7 KB, above the EVM 24 KB deployed-contract limit. The experiment is preserved on `on-chain-verifier`. |
| Failed in-Mini-App prover experiment | Intended to be the World Mini App / browser WebView. The `mini-app-prover` branch added `PROVER_MODE=miniapp`, built the spend statement client-side, and attempted to call `window.CreditzProveKit`. | Backend API, via `/api/spend`, because client-side verification would not be security-relevant. | Backend database or `CreditRegistry.sol`, depending on `LEDGER_MODE`. | This did not produce proofs because the current ProveKit CLI path is Node/server-native and no browser-compatible ProveKit prover bundle was available to install as `window.CreditzProveKit`. The branch intentionally fails closed instead of falling back to backend proving. |
...
The failed Mini App prover experiment is preserved on the `mini-app-prover` branch. To make browser-side proving work later, Creditz needs a real browser/WASM ProveKit prover package, packaged proving keys that fit World App WebView constraints, and performance testing on mobile devices. Until then, the working MVP should use `PROVER_MODE=backend`.
