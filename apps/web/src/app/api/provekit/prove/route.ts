...
function runProveKit(args: string[]) {
  ...
  const result = spawnSync(cli, args, { encoding: "utf8", timeout: 120_000 });
  if (result.error) throw result.error;
  if (result.status !== 0) throw new Error(result.stderr || result.stdout || "ProveKit proof generation failed.");
}
